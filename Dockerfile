...
ENV VF_CONTAINER_BUILD=20260228a

# Create workspace directory
RUN mkdir -p /workspace

# Install claude-automation-recommender skill
# COPY is disabled on CF Sandboxes — embed content via heredoc instead
RUN mkdir -p /root/.claude/commands && \
    cat > /root/.claude/commands/claude-automation-recommender.md << 'SKILL_CAR_EOF'
---
...
- **Recommend 1-2 of each type**: Do not overwhelm - surface the top 1-2 most valuable automations per category
- **If user asks for a specific type**: Focus only on that type and provide more options (3-5 recommendations)
- **Go beyond the reference lists**: Use codebase signals to make tailored recommendations
...
Format recommendations clearly. **Only include 1-2 recommendations per category** - the most valuable ones for this specific codebase. Skip categories that are not relevant.
...
# Embed Groq background analysis agent (runs during pause, writes report for Claude)
# IMPORTANT: Keep in sync with src/sandbox-scripts/groq-background-agent.js
RUN cat > /opt/claude-agent/groq-background-agent.js << 'GROQ_AGENT_EOF'
#!/usr/bin/env node
