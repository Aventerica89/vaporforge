...
4. **DEV_CHANGELOG on every commit.** Before every `git commit`, add a new entry to `DEV_CHANGELOG` in `ui/src/lib/version.ts` with today's date and a one-line summary. This is separate from `CHANGELOG` (feature releases only). Stage the file before committing. Note: `build-info.ts` is auto-generated by `build:info` — you do not manually edit it; `version.ts` is the manual step.
