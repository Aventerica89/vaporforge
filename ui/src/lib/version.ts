...
export const DEV_CHANGELOG: readonly DevLogEntry[] = [
  { date: '2026-03-01', summary: 'fix(github): pass GITHUB_TOKEN in repo fetch requests — unauthenticated calls share CF Worker IP rate limit (60/hr); token bumps to 5000/hr per token' },
  ...
  { date: '2026-02-27', summary: 'feat(container): auto-git autosave — after every Claude response and before idle exit, ws-agent-server commits all workspace changes to a vf-autosave branch and force-pushes; prevents work loss on container eviction; respects .gitignore; non-fatal if no git remote; container build 20260227g' },
  { date: '2026-02-27', summary: 'fix(terminal): "Waiting for sandbox..." dim indicator after 2s of no output; cleared with ANSI \\r\\x1b[2K when first chunk arrives (cold-start UX)' },
  { date: '2026-02-27', summary: 'fix(terminal): execStream first-byte timeout — add 10s AbortController to execStream in XTerminal; aborts and shows "Command timed out" if sandbox never sends data (dead sandbox); once first chunk arrives timeout is cancelled so long-running commands are unaffected; add signal param to sessionsApi.execStream' },
  ...
  { date: '2026-02-27', summary: 'feat(groq-agent): background code reviewer runs during pause — Groq llama-3.3-70b scans git changes + TODOs, writes /workspace/.vf-background-report.md; injected into next Claude prompt on resume; requires GROQ_API_KEY in user secrets; container build 20260227d' },
  { date: '2026-02-27', summary: 'feat(sentinel): workspace keepalive — DO alarm pings sandbox every 8 min to prevent container idle eviction; started on session create/resume, stopped on sleep/delete; fixes product-breaking /workspace wipe on 10-min idle' },
  { date: '2026-02-27', summary: 'fix(v15): container crash detection — capture Process from startProcess(); watchProcessCrash() races waitForExit() against bridge; non-zero exit or SSE stream error closes bridge immediately with error event instead of waiting 5-min bridge timeout' },
  { date: '2026-02-27', summary: 'fix(v15): prevent "Stream stopped" on cold container wake — DO emits connected event immediately on request receipt; container emits ping after CLI pre-flight; both reset frontend 5-min AbortController timer; bump container build to 20260227c' },
  ...
  { date: '2026-02-28', summary: 'fix(v15): bump JWT TTL 360s→660s to cover 10-min bridge timeout (was stale from 5-min era); fix CLAUDE_CONFIG_DIR /root/.config/claude→/root/.claude (wrong path caused heavy skills to silently fail — agents/session state not found)' },
  ...
  { date: '2026-02-22', summary: 'refactor(ui): strip old prompt-input base styles — form base reduced to "relative", remove safe-bottom/border-t/bg-card/80/backdrop-blur-sm/px-4/pb-3/pt-2 conflicts; fix chat-state wrapper px-4 pb-4 on outer (was px-0 edge-to-edge); remove redundant pl-4 pt-3 from textarea className; delete 4 dead sub-components (PromptInputTools, PromptInputActions, PromptInputModeToggle, PromptInputReforge)' },
  { date: '2026-02-22', summary: 'feat(ui): port playground message components into live Claude stream — MessageContent.tsx now uses prompt-kit Reasoning (auto-open/close on stream), Tool (input-streaming→output-available state via tool-start/result pair lookup), CodeBlockWithCopy for artifacts, Steps+TextShimmer for chain-of-thought; remove ReasoningBlock+ArtifactBlock+ChainOfThoughtBlock+ToolCallBlock from parts renderer' },
  ...
  { date: '2026-02-22', summary: 'feat(playground): wire FeedbackBar + ThinkingBar into ChatPreview; add rich markdown showcase message (h2/h3/bold/italic/list/code/blockquote); replace PulseLoader+TextShimmer streaming indicator with ThinkingBar' },
  ...
  { date: '2026-02-22', summary: 'fix(playground): PlaygroundPage responsive layout — md:justify-center on outer column + md:flex-none on heading area restores vertical center on desktop while keeping mobile pinned-bottom behavior (flex-1 min-h-0 only applies on mobile)' },
  ...
  { date: '2026-02-22', summary: 'Replace ai-elements Shimmer with prompt-kit TextShimmer in ReasoningBlock for consistent CSS-based shimmer animation' },
  { date: '2026-02-22', summary: 'Add FeedbackPrompt to ChatPanel: shows FeedbackBar after each streaming response completes, dismissable via thumbs or close' },
];
