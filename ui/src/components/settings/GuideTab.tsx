...
type SubTabId = 'start' | 'chat' | 'config' | 'extensions' | 'dev';

export function GuideTab() {
  const [activeTab, setActiveTab] = useState<SubTabId>('start');
      ...
      <div className="flex gap-1 overflow-x-auto border-b border-border mb-6">
        {SUB_TABS.map((tab) => (
          <button
            ...
            className={`shrink-0 px-3 py-2 text-[11px] font-medium transition-colors border-b-2 -mb-px ${
              activeTab === tab.id
          ...
          <>
            <Section icon={<MessageSquare className="h-4 w-4 text-primary" />} title="Chat & AI Tools">
              ...
              <p className="text-xs text-muted-foreground/60 leading-relaxed">
                Code Transform requires a Claude API key
            ...
            <Section icon={<Sparkles className="h-4 w-4 text-primary" />} title="Quick Chat">
              ...
              <div className="space-y-1.5 text-sm text-muted-foreground">
                ...
                <ul className="space-y-1">
                  <Tip>
                    <strong className="text-foreground">Instant mode</strong> — No session required.
                    Ask Claude, Gemini, or OpenAI anything and get a fast response.
                  </Tip>
                  <Tip>
                    <strong className="text-foreground">Agent mode</strong> — Active when a sandbox session is open.
                    Can read files, search code semantically, run commands, and create GitHub issues.
                  </Tip>
                </ul>
              </div>
              <div className="space-y-1.5 text-sm text-muted-foreground">
                <p className="font-medium text-foreground text-xs">Slash commands — type <code className="text-primary">/</code> to see options:</p>
                <ul className="space-y-1">
                  <Tip><code className="text-primary">/explain</code> — Explain how this codebase works</Tip>
                  <Tip><code className="text-primary">/bugs</code> — Find potential bugs and security issues</Tip>
                  <Tip><code className="text-primary">/tests</code> — Write unit tests for critical functions</Tip>
                  <Tip><code className="text-primary">/optimize</code> — Identify performance bottlenecks</Tip>
                  <Tip><code className="text-primary">/issue</code> — Create a GitHub issue (agent mode)</Tip>
                  <Tip><code className="text-primary">/docs</code> — Generate documentation for main modules</Tip>
                </ul>
              </div>
              <div className="rounded-lg border border-border bg-muted/50 p-3 text-xs text-muted-foreground leading-relaxed space-y-1">
                <p><strong className="text-foreground">Provider/model:</strong> Switch between Claude, Gemini, and OpenAI in the toggle bar. Select the specific model from the dropdown.</p>
                <p><strong className="text-foreground">Setup:</strong> Add API keys in Settings &gt; Secrets (ANTHROPIC_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY).</p>
                <p><strong className="text-foreground">Pro tips:</strong> Ask it to push bugs to GitHub, explain an unfamiliar codebase, or write tests for a specific file.</p>
              </div>
            </Section>
