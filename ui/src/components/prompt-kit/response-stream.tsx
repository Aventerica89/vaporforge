...
function ResponseStream({
  ...
  const fadeDurationMs = getFadeDuration();
  const segmentDelayMs = getSegmentDelay();
  ...
  const fadeStyle = [
    ...
    `  opacity: 0;`,
    `  animation: fadeIn ${fadeDurationMs}ms ease-out forwards;`,
    `}`,
    `.fade-segment-space { white-space: pre; }`,
  ].join('\n');
              ...
              {segments.map((segment, idx) => {
                ...
                return (
                  <span
                    ...
                    style={{ animationDelay: `${idx * segmentDelayMs}ms` }}
